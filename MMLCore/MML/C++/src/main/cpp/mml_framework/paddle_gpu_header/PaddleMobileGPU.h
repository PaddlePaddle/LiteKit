 ...
 * 是否是量化模型
 */
@property (assign, nonatomic) bool quantized;

/*
 * n-fold量化参数
 */
@property (assign, nonatomic) int quantificationFold;

@end

@interface PaddleMobileGPU: NSObject

-(void)getTextureWithBuffer:(id<MTLBuffer>)buffer withCompletion:(void (^)(id<MTLTexture>))completion;

-(void)getTextureWithBuffer:(id<MTLBuffer>)buffer withCompletion:(void (^)(id<MTLTexture>))completion
                 channelNum:(NSInteger)channelNum;
...
-(void)predictBuffer:(id<MTLBuffer>)buffer channleNum: (NSInteger)channelNum withResultCompletion:(void (^)(BOOL, NSArray <PaddleMobileGPUResult *> *))completion;

-(void)predictBuffers:(NSArray<id<MTLBuffer>> *)buffers  withResultCompletion:(void (^)(BOOL, NSArray <PaddleMobileGPUResult *> *))completion;

-(void)predictBatch:(NSArray<id<MTLBuffer>> *)buffers channleNum: (NSInteger)channelNum withResultCompletion:(void (^)(BOOL, NSArray<PaddleMobileGPUResult *> *))completion;
